...
class IrelandOfferListView(generics.ListAPIView):
    serializer_class = IrelandOfferListSerializer
    permission_classes = (IsConsumer,)
    queryset = Offer.objects.filter(is_active=True).order_by('-priority')
...
class OfferBannerView(generics.ListAPIView):
    ...
    queryset = Offer.objects.filter(is_active=True, is_promo=True).order_by('-priority')
...
class IrelandOfferApplyView(generics.GenericAPIView, ValidateSerializerMixin):
    serializer_class = OfferApplySerializer
    permission_classes = (IsConsumer,)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if offer.max_booking_amount > 0:
            if cart.subtotal >= offer.max_booking_amount:
                return response.Response({
                    'error': 'Offer cannot be applied on bookings greater than EUR.{}'.format(offer.max_booking_amount)
                }, status=status.HTTP_400_BAD_REQUEST)

            if cart.subtotal < offer.min_booking_amount:
                return response.Response({
                    'error': 'Offer cannot be applied on bookings less than EUR.{}'.format(offer.min_booking_amount)
                }, status=status.HTTP_400_BAD_REQUEST)
